...
class SevenSegmentWeightedObjective:
    ...
    def _collision_violation(self, trajectory: Tuple[np.ndarray, np.ndarray, np.ndarray]) -> float:
        """硬约束违反度：防碰不足量 (m² 量纲)，0 表示无碰撞。"""
        violation = 0.0
        for well_obstacle in self.well_obstacles:
            ...
            if d_min < safe:
                violation += (safe - d_min + 1.0) ** 2
        return float(violation)

    def _collision_penalty(self, trajectory: Tuple[np.ndarray, np.ndarray, np.ndarray]) -> float:
        violation = self._collision_violation(trajectory)
        if violation <= 0.0:
            return 0.0
        return float(self.hard_collision_penalty + self.w_collision_hard * violation)

    def constraint_violation(self, position_tuple: Any) -> float:
        """硬约束违反度（参数可行 + 防碰），供约束支配比较使用。"""
        try:
        ...
        return self._collision_violation((traj["E"], traj["N"], traj["D"]))

    def evaluate_pair(self, position_tuple: Any) -> Tuple[float, float]:
        """一次轨迹计算返回 (目标值, 约束违反度)。"""
        try:
            params = self._to_param_dict(position_tuple)
        except Exception:
        ...
        cv = self._collision_violation(trajectory)
        collision_term = self._collision_penalty(trajectory)
        ...
        if dev > self.hit_threshold:
            objective = float(self.w_target * (dev ** 2) + collision_term)
        else:
            objective = float(self.w_length * float(traj["total_length"]) + collision_term)
        return objective, cv

    def calculate_objective(self, position_tuple: Any) -> float:
        return self.evaluate_pair(position_tuple)[0]
