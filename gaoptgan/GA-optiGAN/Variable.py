...
hyperparameter_defaults = dict(
    ...
    gen_n = 10,
    elite_ratio = 0.5,
    # 探索 warm-up：此期间不收缩 archive、不启用 D_R/hybrid，延续预训练的全局搜索
    exploration_warmup_epochs = 10,
    # D_R 训练优先使用可行解；不足时回退到全 archive
    dr_train_feasible_only = True,
    dr_train_min_feasible = 2,
)

# para:show
En_SCALE = False
ENABLE_LOGGING = True
CV_FEASIBLE_TOL = 1e-9
...
def init_history(problem, para_dict):
    ...
    fitness_history = np.zeros((para_dict.init_size, 1))
    cv_history = np.zeros((para_dict.init_size, 1))
    data_history = []
    ...
    for i in range(init_size):
        fitness_history[i, :], cv_history[i, :] = _problem_evaluate(problem, data_history[i, :])
    return data_history.astype(np.float32), fitness_history.astype(np.float32), cv_history.astype(np.float32)


def _problem_evaluate(problem, x):
    if hasattr(problem, "evaluate_individual"):
        fitness, cv = problem.evaluate_individual(x)
    else:
        fitness = problem(x)
        cv = 0.0
    return float(fitness), float(cv)
...
class Problem:
    def __init__(self, func, evaluate_pair=None, infeasible_fitness_threshold=1e19):
        self.maxFes = 0
        self.solutions = []
        self.fitness = []
        self.func = func
        self.evaluate_pair_fn = evaluate_pair
        self.infeasible_fitness_threshold = float(infeasible_fitness_threshold)

    def evaluate_individual(self, x):
        self.maxFes += 1
        self.solutions.append(x)
        if self.evaluate_pair_fn is not None:
            fit, cv = self.evaluate_pair_fn(x)
        else:
            fit = self.func(x)
            cv = 0.0 if fit < self.infeasible_fitness_threshold else fit - self.infeasible_fitness_threshold
        self.fitness.append(fit)
        return float(fit), float(cv)

    def __call__(self, x):
        fit, _ = self.evaluate_individual(x)
        return fit
