...
def plot_population_boxplot_from_csv(csv_path="ga_population_all_individuals.csv",
    ...
    for idx, name in enumerate(PARAM_NAMES):
        ...
        axes[idx].yaxis.set_major_formatter(yfmt)
...
def generate_seven_segment_trajectory(custom_params: Optional[Dict[str, float]] = None,
    ...
    # 工具能力上限：由 DLS_turn 与 L4 给出可实现最大扭角
    sin_alpha = max(np.sin(np.radians(max(alpha3, 1e-3))), 1e-3)
    dphi_cap = (L4 * abs(k_turn)) / sin_alpha
    dphi_turn = float(np.clip(dphi_target, -dphi_cap, dphi_cap))
    # 第6段井斜调整长度（增/降皆可）
    ...
    # 4) 专用扭方位圆弧段（井斜保持 alpha3）
    n = max(1, int(np.ceil(L4 / ds))) if L4 > 0 else 0
    for k in range(1, n + 1):
        frac = k / n
        azi = phi_init + dphi_turn * frac
        append_step(L4 / n, alpha3, azi)
    ...
    return {
        "params": {
            ...
            "DLS_turn": DLS_turn, "L4": L4, "dphi_turn": dphi_turn, "L5": L5,
