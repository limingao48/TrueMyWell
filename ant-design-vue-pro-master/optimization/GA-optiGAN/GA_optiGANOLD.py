...
def genetic_operations(population, para_dict, crossover_rate, mutation_rate):
    ...
    for i in range(0, len(population), 2):
        if i+1 < len(population) and np.random.random() < crossover_rate:
    ...
    for indiv in offspring:
        if np.random.random() < mutation_rate:
            mutation_mask = np.random.random(dim) < 0.2
            noise = np.random.normal(0, 0.1, dim) * mutation_mask
            indiv += noise
            
            indiv = np.clip(indiv, bounds[:, 0], bounds[:, 1])
...
def GA_optiGAN_min(
    ...
    GA_optiGAN_module = GA_optiGANMD(
    ...
    print("Start pretraining, pretrain%d rounds"%(para_dict.pretrain))
    # pretrain
    for i in range(para_dict.pretrain):
        GA_optiGAN_module.pretrain(
    ...
    for epoch in range(maxfun):
        ...
        if epoch % gen_n == 0 and epoch > 0:
            # Select the elite solution (top elite_ratio)
            elite_ratio = config_init.elite_ratio
            elite_size = max(1, int(len(data_history) * elite_ratio))
            elite_indices = np.argsort(fitness_history.reshape(-1))[:elite_size]
            elite_solutions = data_history[elite_indices].copy()
            
            # Perform genetic manipulation (crossover + mutation)
            ...
            if offspring.size != 0:
                # Merge solution sets and filter for the best solution
                merged_data = np.vstack([data_history, offspring])
                merged_fitness = np.vstack([fitness_history, offspring_fitness])
                sorted_indices = np.argsort(merged_fitness.reshape(-1))[:len(data_history)]
                data_history = merged_data[sorted_indices]
                fitness_history = merged_fitness[sorted_indices]
                current_best_idx = np.argmin(fitness_history)
                current_best_solution = data_history[current_best_idx]
                current_best_fitness = fitness_history[current_best_idx][0]
                
                print(f"[After genetic manipulation] epoch={epoch}, best_fitness=%.3e, best_solution=%s" % (
                    current_best_fitness, current_best_solution
                ))
            else:
                print("Offspring is empty")
        if En_SCALE:
        ...
        upper = (para_dict.upper - data_mean) / data_std
        lower = (para_dict.lower - data_mean) / data_std
        NN_dict = GA_optiGAN_module.train(
            train_data_torch,
            para_dict.epoch,
            para_dict.D_iter,
            upper,
        ...
        z_fake = GA_optiGAN_module.generator(noise)
        ...
        elapsed_time = time.time() - run_time_start 
        ...
        if stop == True:
            ...
            print(
                "epoch=%d, fes=%d,curr_bestf=%.3e,bestf=%.3e,distance_history=%.2e,distance_gen=%.2e,distance_train=%.2e,D1_loss=%.3e, D2_loss=%.3e,D1_Wasserstein=%.3e,D1_Wasserstein=%.3e,G_loss=%.3e,elapsed_time=%.2f秒,bestpoint_x=%s\n"
                % (
                    epoch + 1,
                    evaluations,
                    curr_bestf[epoch],
