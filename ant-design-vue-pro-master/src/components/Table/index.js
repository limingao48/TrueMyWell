...
export default {
  ...
  created () {
    ...
    console.log(this.showSizeChanger)
    this.needTotalList = this.initTotalList(this.columns)
    this.loadData()
  },
  methods: {
    ...
    loadData (pagination, filters = this.filters, sorter = this.sorter) {
      ...
      this.sorter = sorter
      console.log('输出pagination')
      console.log(pagination)
      this.localLoading = true
      const parameter = Object.assign({
        number: (pagination && pagination.current) ||
      ...
      if ((typeof result === 'object' || typeof result === 'function') && typeof result.then === 'function') {
        result.then(r => {
          console.log('输出r.data')
          console.log(r.content)
          const pageable = r.pageable
          console.log(pageable.pageSize)
          this.localPagination = this.showPagination && Object.assign({}, this.localPagination, {
            current: pageable.offset / pageable.pageSize, // 返回结果中的当前分页数
            total: r.totalElements, // 返回结果中的总记录数
            showSizeChanger: this.showSizeChanger,
            pageSize: pageable.pageSize
          }) || false
          // 为防止删除数据后导致页面当前页面数据长度为 0 ,自动翻页到上一页
          if (r.content.length === 0 && this.showPagination && this.localPagination.current > 1) {
